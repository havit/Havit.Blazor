...
{
	/// <summary>
	/// Scrolls to the anchor on <c>firstRender</c> and whenever the location changes (<see cref="NavigationManager.LocationChanged"/>).
	...
	/// Use <see cref="SamePage"/> or <see cref="Manual"/> when the page contents load asynchronously and the layout changes.
	/// </summary>
	Full = 0,

	/// <summary>
	/// Explicit calls to <see cref="HxAnchorFragmentNavigation.ScrollToCurrentUriFragmentAsync()"/> or <see cref="HxAnchorFragmentNavigation.ScrollToAnchorAsync(string)"/> are needed.
	...
	/// <summary>
	/// Same as <see cref="Manual"/>, but scrolls to the anchor on <see cref="NavigationManager.LocationChanged"/> if the page remains the same (just the #fragment portion changed).
