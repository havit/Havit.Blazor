...
public record class ApplicationInsightsConfig : ApplicationInsightsConfiguration
{
	...
	/// <inheritdoc cref="ApplicationInsightsConfiguration.MergeTo(ApplicationInsightsConfiguration)"/>
	internal override void MergeTo(ApplicationInsightsConfiguration target)
	{
		base.MergeTo(target);

		var typedTarget = (ApplicationInsightsConfig)target;
		typedTarget.EmitLineDelimitedJson ??= EmitLineDelimitedJson;
		typedTarget.AccountId ??= AccountId;
		typedTarget.SessionRenewalMs ??= SessionRenewalMs;
		typedTarget.SessionExpirationMs ??= SessionExpirationMs;
		typedTarget.MaxBatchSizeInBytes ??= MaxBatchSizeInBytes;
		typedTarget.MaxBatchInterval ??= MaxBatchInterval;
		typedTarget.DisableExceptionTracking ??= DisableExceptionTracking;
		typedTarget.DisableTelemetry ??= DisableTelemetry;
		typedTarget.SamplingPercentage ??= SamplingPercentage;
		typedTarget.AutoTrackPageVisitTime ??= AutoTrackPageVisitTime;
		typedTarget.EnableAutoRouteTracking ??= EnableAutoRouteTracking;
		typedTarget.DisableAjaxTracking ??= DisableAjaxTracking;
		typedTarget.DisableFetchTracking ??= DisableFetchTracking;
		typedTarget.OverridePageViewDuration ??= OverridePageViewDuration;
		typedTarget.MaxAjaxCallsPerView ??= MaxAjaxCallsPerView;
		typedTarget.DisableDataLossAnalysis ??= DisableDataLossAnalysis;
		typedTarget.DisableCorrelationHeaders ??= DisableCorrelationHeaders;
		typedTarget.DistributedTracingMode ??= DistributedTracingMode;
		typedTarget.CorrelationHeaderExcludedDomains ??= CorrelationHeaderExcludedDomains;
		typedTarget.DisableFlushOnBeforeUnload ??= DisableFlushOnBeforeUnload;
		typedTarget.DisableFlushOnUnload ??= DisableFlushOnUnload;
		typedTarget.EnableSessionStorageBuffer ??= EnableSessionStorageBuffer;
		typedTarget.IsCookieUseDisabled ??= IsCookieUseDisabled;
		typedTarget.IsRetryDisabled ??= IsRetryDisabled;
		typedTarget.Url ??= Url;
		typedTarget.IsStorageUseDisabled ??= IsStorageUseDisabled;
		typedTarget.IsBeaconApiDisabled ??= IsBeaconApiDisabled;
		typedTarget.DisableXhr ??= DisableXhr;
		typedTarget.OnunloadDisableFetch ??= OnunloadDisableFetch;
		typedTarget.SdkExtension ??= SdkExtension;
		typedTarget.IsBrowserLinkTrackingEnabled ??= IsBrowserLinkTrackingEnabled;
		typedTarget.AppId ??= AppId;
		typedTarget.EnableCorsCorrelation ??= EnableCorsCorrelation;
		typedTarget.NamePrefix ??= NamePrefix;
		typedTarget.SessionCookiePostfix ??= SessionCookiePostfix;
		typedTarget.UserCookiePostfix ??= UserCookiePostfix;
		typedTarget.EnableRequestHeaderTracking ??= EnableRequestHeaderTracking;
		typedTarget.EnableResponseHeaderTracking ??= EnableResponseHeaderTracking;
		typedTarget.EnableAjaxErrorStatusText ??= EnableAjaxErrorStatusText;
		typedTarget.EnableAjaxPerfTracking ??= EnableAjaxPerfTracking;
		typedTarget.MaxAjaxPerfLookupAttempts ??= MaxAjaxPerfLookupAttempts;
		typedTarget.AjaxPerfLookupDelay ??= AjaxPerfLookupDelay;
		typedTarget.OnunloadDisableBeacon ??= OnunloadDisableBeacon;
		typedTarget.AutoExceptionInstrumented ??= AutoExceptionInstrumented;
		typedTarget.CorrelationHeaderDomains ??= CorrelationHeaderDomains;
		typedTarget.AutoUnhandledPromiseInstrumented ??= AutoUnhandledPromiseInstrumented;
		typedTarget.EnableUnhandledPromiseRejectionTracking ??= EnableUnhandledPromiseRejectionTracking;
		typedTarget.CustomHeaders ??= CustomHeaders;
		typedTarget.EventsLimitInMem ??= EventsLimitInMem;
		typedTarget.DisableIkeyDeprecationMessage ??= DisableIkeyDeprecationMessage;
		typedTarget.DisableUserInitMessage ??= DisableUserInitMessage;
		typedTarget.AddIntEndpoints ??= AddIntEndpoints;
		typedTarget.UserOverrideEndpointUrl ??= UserOverrideEndpointUrl;
	}
}
