...
Configure **options** in the project where `<HxApplicationInsightsScript>` is rendered.
...
Place `<HxApplicationInsightsScript />` inside the `<head>` element of your root layout or `App.razor`,
...
<head>
    ...
    <HxApplicationInsightsScript @rendermode="..." />
    ...
</head>
```
...
Use `<HxApplicationInsightsException>` inside an `ErrorBoundary` to report caught exceptions automatically:
...
<ErrorBoundary>
    ...
    <ErrorContent Context="ex">
        <HxApplicationInsightsException Exception="ex" />
...
```razor
<HxApplicationInsightsScript Nonce="@cspNonce" />
```
