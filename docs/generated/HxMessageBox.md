...
|------|------|-------------|
| AdditionalAttributes | `Dictionary<string, object>` | Additional attributes to be splatted onto an underlying `HxDialog`. |
...
| ShowCloseButton | `bool?` | Indicates whether to show the close button. The default is taken from the underlying `HxDialog` (`true`). |
