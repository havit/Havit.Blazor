...
public partial class HxEChart : IAsyncDisposable
{
	...
	/// </summary>
	/// <remarks>
	/// The lookup is case-insensitive even though it does not look like it: the framework builds the
	/// <see cref="AdditionalAttributes"/> dictionary with an <see cref="StringComparer.OrdinalIgnoreCase"/> comparer,
	/// so <c>Id</c> or <c>ID</c> is matched as well (and Blazor eliminates the duplicate <c>id</c> attribute case-insensitively too).
	/// </remarks>
	private string ChartIdEffective
	{
		get
		{
			if ((AdditionalAttributes is not null)
				&& AdditionalAttributes.TryGetValue("id", out var splattedId)
				&& !String.IsNullOrEmpty(splattedId?.ToString()))
			{
				return splattedId.ToString();
			}
			return ChartId;
		}
	}
