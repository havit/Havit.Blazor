...
public class GridInternalStateSortingItemHelperTests
{
	...
	public void GridInternalStateSortingItemHelper_ApplyColumnToSorting_OnEmptySorting()
	{
		...
		// Assert
		Assert.Single(newSorting);
		Assert.Same(mockColumn.Object, newSorting.Single().Column);
		Assert.False(newSorting.Single().ReverseDirection);
	}
	...
	public void GridInternalStateSortingItemHelper_ApplyColumnToSorting_TakesColumnToTheFirstPosition()
	{
		...
		// Assert
		Assert.Equal(2, newSorting.Count());

		Assert.Same(mockColumn2.Object, newSorting[0].Column);
		Assert.False(newSorting[0].ReverseDirection);

		Assert.Same(mockColumn1.Object, newSorting[1].Column);
		Assert.False(newSorting[1].ReverseDirection);
	}
	...
	public void GridInternalStateSortingItemHelper_ApplyColumnToSorting_TogglesReverseOnFirstColumn()
	{
		...
		Assert.Same(mockColumn1.Object, newSorting[0].Column);
		Assert.True(newSorting[0].ReverseDirection);
	}
	...
	public void GridInternalStateSortingItemHelper_ToSortingItems_TwoSimpleColumns()
	{
		...
		// Assert
		Assert.Equal(2, sortingItems.Count());
		Assert.Equal("A", sortingItems[0].SortString);
		Assert.Equal(SortDirection.Ascending, sortingItems[0].SortDirection);
		Assert.Equal("B", sortingItems[1].SortString);
		Assert.Equal(SortDirection.Ascending, sortingItems[1].SortDirection);
	}
